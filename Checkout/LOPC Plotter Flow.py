'''
6/18/24 update: LOPC_time[1:]  ->  LOPC_time[-150:]
'''
...
def make_LOPC_fig(cal_val, log_lin):  #Plot the CN Counts
    ...
    try:
        ...
        plt.plot(LOPC_time[-150:], Pump1_I, 'k-', label='Pump1 I')       #plot the channels
        plt.plot(LOPC_time[-150:], Pump2_I, 'r-', label='Pump2 I')
        plt.legend(loc='upper left', fontsize = 'x-small')
        plt.xlabel('Elapsed Time [s]', fontsize = 'x-small')
        plt.tight_layout()
        
        plt2=plt.twinx()
        plt2.plot(LOPC_time[-150:], Pump1_T, 'b-', label='Pump1 T')
        plt2.plot(LOPC_time[-150:], Pump2_T, 'g-', label='Pump2 T')
        ...
        plt.grid(True)                                  #Turn the grid on
        plt.ylabel('#/cc')                            
        plt.plot(LOPC_time[-150:], sum300, 'k-', label='300nm')
        plt.plot(LOPC_time[-150:], sum500, 'r-', label='500nm')
        plt.plot(LOPC_time[-150:], sum1000, 'b-', label='1000nm')
        plt.plot(LOPC_time[-150:], sum2000, 'g-', label='2000nm')
        if log_lin == True:
            plt.yscale('log')
        plt.legend(loc='upper left', fontsize = 'x-small')
        plt.tight_layout()
...
def decode_string(strng, saveFile, filename):
    ...
    if len(d) == 535 or len(d) == 538:
        ...
        Flow.append(float(d[19]))
        Pump1_BEMF.append(float(d[20]))
        Pump1_PWM.append(float(d[21]))
        Pump2_BEMF.append(float(d[22]))
        Pump2_PWM.append(float(d[23]))
        LOPC_time.append(LOPC_time[-1]+2)

        global HG_Array
        global LG_Array
        HG_Array = np.array(list(map(float,d[24:279])),dtype = float)
        LG_Array = np.array(list(map(float,d[280:534])),dtype = float)
...
def main():
    ...
    window2 = sg.Window('LOPC House Keeping', layout_hk)
    ...
    while True: # While loop that loops forever
        ...
        if decode_string(LOPCString,saveFile,filename) == True:
            ...
            window2.Element('_Flow_').Update(Flow[-1])
